...
if($_SERVER['REQUEST_METHOD'] == 'POST'){ // Atende a somente formulário com method GET
    ...
    if(isset($_POST['registrar_corredores'])){
        try{
            ...
            //Obtém informações sobre a categoria cadastrada
            $service_corredor->obterRelatorioCategoriaCorrida($pdo, $categoria);
        ...
        try{
            ...
            //Obtém informações sobre a categoria após registrar chegada
            $service_corredor->obterRelatorioCategoriaCorrida($pdo, $categoria);
       ...
       try{ 
            ...
            $service_csv->limparArquivo($categoria);
            $service_csv->registrarCorredores($pdo, $categoria);
        }catch(PDOException $pdoe){ // Caso ocorra alguma exceção no pdo ou banco de dados
           ...
           // Caso tenha alguma transação cancela ela
            ...
            // Registra nas logs o erro ocorrido
            $exception_service->registrarCorredorErros($e);
            $_SESSION['mensagem'] = "Erro ao exportar os dados para o aquivo .csv";
            // Indica que houve um erro interno
            http_response_code(500);
    ...
    if(isset($_GET['trocar_corrida'])){
            try{
                ...
                // Obtém os dados da nova categoria
                $service_corredor->obterRelatorioCategoriaCorrida($pdo, $categoria);
